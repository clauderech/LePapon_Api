...
class PDFGenerator:
    ...
    def gerar_pdf_completo(self, dados, titulo, campos_exibir, colunas_legenda, larguras_colunas, pasta_destino, nome_arquivo):
        """
        Gera um PDF completo com cabeçalho, tabela e total
        """
        os.makedirs(pasta_destino, exist_ok=True)
        pdf_path = os.path.join(pasta_destino, nome_arquivo)
        
        c = canvas.Canvas(pdf_path, pagesize=A4)
        
        # Cabeçalho
        y = self.criar_cabecalho(c, titulo)
        
        # Tabela
        y, total = self.criar_tabela(c, y, colunas_legenda, larguras_colunas, dados)
        
        # Finalizar
        self.finalizar_pdf(c, y, total)
