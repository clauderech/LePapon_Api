...
class EnviarContaCliente:
    ...
    def delete_pdf_droplet(self, host, usuario, caminho_remoto, caminho_chave):
        """
        Remove um arquivo do droplet via SFTP usando chave SSH.
        - host: IP ou domínio do droplet
        - usuario: usuário SSH
        - caminho_remoto: caminho do arquivo no droplet a ser removido
        - caminho_chave: caminho da chave privada SSH
        """
        try:
            key = paramiko.RSAKey.from_private_key_file(caminho_chave)
            ...
            if sftp is None:
                transport.close()
                raise ConnectionError("Falha ao criar o cliente SFTP. Verifique as credenciais e a conexão.")
            sftp.remove(caminho_remoto)
            sftp.close()
            transport.close()
            return True
        except Exception as e:
            print(f"Erro ao remover arquivo remoto: {e}")
            return False
